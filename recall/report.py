...
def build_report(
    ...
    """Render `results` (one per scored query clip) as a self-contained HTML file at `out_path`."""
    mean_score = (
    ...
    doc = (
        ...
        f"<p>Mean score: <strong>{mean_score:.4f}</strong> over "
        f"{len(results)} scene(s)</p>"
        f"{body}</body></html>"
    )
...
def _query_section(r: QueryResult) -> str:
    ...
    return (
        ...
        f"{r.score:.4f}</h2>"
        '<div class="row">'
        f"{_clip_card(r.clip, r.curve, None)}"
        f'<div class="neighbors">{neighbors_html}</div>'
        "</div></section>"
    )


def _clip_card(
    clip: str, curve: RDCurve, similarity: float | None
) -> str:
    label = (
        "query"
...
_CSS = (
    ...
    ".row,.neighbors{display:flex;gap:1rem;flex-wrap:wrap}"
    ".card{border:1px solid #ccc;border-radius:8px;padding:.5rem;width:160px}"
    ".card img{width:100%;border-radius:4px}"
    ".label{font-size:.8rem;color:#555}"
    "table.curve{width:100%;font-size:.75rem;margin-top:.25rem}"
    "section.query{margin-bottom:2rem;border-bottom:1px solid #eee}"
)
