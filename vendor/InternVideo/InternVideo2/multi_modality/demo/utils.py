...
# ponytail: vendored tokenization_bert.py predates transformers' internal tokenizer
# refactor (self.vocab / get_vocab no longer compatible) - stock BertTokenizer
# is the same WordPiece tokenizer and works with the same vocab.txt.
...
def frames2tensor(vid_list, fnum=8, target_size=(224, 224), device=torch.device('cuda')):
    ...
    # ponytail: cast to float32 before moving to device - MPS doesn't support float64
    vid_tube = torch.from_numpy(vid_tube).float().to(device, non_blocking=True)
    return vid_tube
